...
def save_profile(telegram_id: int, data: dict) -> dict:
    ...
    if error:
        logger.warning(
            "Ошибка валидации анкеты пользователя %s: %s", telegram_id, error
        )
    ...
    logger.info(
        "Профиль пользователя %s %s", telegram_id, "создан" if created else "обновлён"
    )
    return {
...
def get_random_profile(exclude_telegram_id: int) -> Optional[dict]:
    ...
    logger.info(
        "Пользователь %s получил рандомную анкету %s",
        exclude_telegram_id,
        profile.telegram_id,
    )
    return {
...
def get_profiles_list(
    exclude_telegram_id: int, exclude_list: Optional[List[int]] = None
) -> List[dict]:
    exclude_list = exclude_list or []
    profiles = UserProfile.objects.exclude(telegram_id=exclude_telegram_id).exclude(
        telegram_id__in=exclude_list
    )
    logger.info(
        "Пользователь %s запросил список анкет (исключая: %s)",
        exclude_telegram_id,
        exclude_list,
    )
    return [
