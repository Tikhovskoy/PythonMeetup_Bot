...
- Redis и Celery для фоновой отправки рассылок;
- uv для зависимостей;
...
Redis используется как брокер очереди Celery. Рассылки из Django Admin выполняются
отдельным worker-процессом, поэтому интерфейс администратора не ждёт ответа Telegram.
```
...
Контейнерное окружение включает Django, Telegram-бота, PostgreSQL, Redis и Celery worker.
